...
<div id="order-modal" x-data="{ open: true }" x-show="open" x-cloak class="fixed inset-0 z-[150] flex items-center justify-center p-4">
    ...
    <div class="bg-white rounded-[2.5rem] shadow-2xl relative w-full max-w-lg overflow-hidden ring-1 ring-white/20" 
        ...
        <div class="bg-slate-900 text-white p-8 flex justify-between items-center">
            ...
            <button @click="open = false; setTimeout(() => $el.remove(), 300)" class="text-slate-400 hover:text-white transition-colors p-2 bg-white/5 rounded-full">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
        </div>

        <form hx-post="<?= htmlspecialchars(strval($base ?? "")) ?>/order/save" class="p-10 space-y-8">
            <div>
                ...
                <select name="vehicle_id" required class="input-premium">
                    ...
                    <?php foreach ($vehicles ?? [] as $v): ?><option value="<?= htmlspecialchars(strval($v->id ?? "")) ?>"><?= htmlspecialchars(strval($v->plate ?? "")) ?> - <?= htmlspecialchars(strval($v->brand->name ?? "")) ?> <?= htmlspecialchars(strval($v->model ?? "")) ?></option><?php endforeach; ?>
                </select>
            </div>
