...
<div class="card-premium !bg-white p-0 overflow-hidden border-indigo-100 ring-1 ring-indigo-50">
        ...
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-indigo-50/30 transition-colors">
                    <td class="py-5 px-6">
                        <div class="font-black text-slate-800 text-sm"><?= htmlspecialchars(strval($apt->scheduled_date->strftime('%d/%m/%Y') ?? "")) ?></div>
                        <div class="text-[10px] font-black text-indigo-500 font-mono"><?= htmlspecialchars(strval($apt->scheduled_date->strftime('%H:%M') ?? "")) ?>hs</div>
                    </td>
                    <td class="py-5 px-6">
                    ...
                    <td class="py-5 px-6">
                        <span class="bg-slate-100 text-slate-600 px-3 py-1 rounded-lg font-mono font-black text-[10px] border border-slate-200">
                             <?= htmlspecialchars(strval($apt->plate or (apt->vehicle->plate if apt->vehicle else '---') ?? "")) ?>
                        </span>
                    </td>
                    <td class="py-5 px-6">
    ...
    <div class="p-4 bg-slate-900 flex justify-end">
        <button onclick="window.print()" class="btn-primary !py-2 !px-6 !text-[10px] !bg-white !text-slate-900 border-none">
            🖨️ IMPRIMIR REPORTE
        </button>
    </div>
</div>
